...
type AssignQrFlowProps = {
  authToken: string;
};

export function AssignQrFlow({ authToken }: AssignQrFlowProps) {
  ...
  const normalizeQrInput = useCallback((value: string) => {
  ...
  const lookupQrCode = useCallback(async (rawValue: string) => {
    ...
    <>
      ...
      <div className="rounded-3xl bg-white border border-gray-200 p-6">
        ...
        <div className="mt-6 grid gap-4 lg:grid-cols-[1.2fr_1fr]">
          ...
          <div className="rounded-2xl border border-gray-100 bg-gray-50/70 p-4 space-y-4">
            <div className="text-sm font-semibold text-gray-600">Selected QR</div>
            <div className="text-xs text-gray-500">{selectedQr || 'No QR selected yet'}</div>
          </div>
        </div>
      </div>
